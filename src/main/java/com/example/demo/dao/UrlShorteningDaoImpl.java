...
@Repository
public class UrlShorteningDaoImpl implements UrlShorteningDao {
	...
	public int insert(String shorteningKey, String longURL) {
		String sql = "insert into url_map(shortening_key, long_url, create_date, expiry_date) values(?, ?, sysdate, sysdate + "
				+ ddDao.getDDMessage(EXPIRY_DATE) + ")";
	...
	public String getShorteningKey(String longURL) {
		...
		String shorteningKey = jdbcTemplate.query(sql, object, new BeanPropertyRowMapper<UrlMap>(UrlMap.class)).get(0)
				.getShorteningKey();
	...
	public String getLongURL(String shorteningKey) {
		...
		String longURL = jdbcTemplate.query(sql, object, new BeanPropertyRowMapper<UrlMap>(UrlMap.class)).get(0)
				.getLongUrl();
